...
| `$40E5` | `update_gauge_281a` | Draws **both Altimeter #7 bars** (CONFIRMED, rendered on Amiga): (1) on `$281A` change, `$FF`-fills player **`$0C98` (P0)** from `$281A` down to `$38` = the **terrain-height bar** (`$281A` tracks terrain `$0062`, purple `$00D5`). (2) on `$281B` change, steps the **missile M3** top edge — `AND $3F` clears bits 6-7 at `$0B96/$0B97`, `OR $C0` sets them at `$0B98/$0B99` (offset `$281B`) = the **ship-height bar** (`$281B` tracks altitude `$0034`, light-blue `$00D6`). NOT wing clearance. | `draw_altimeter_bars` |
...
- `$2875` `gauge_281a_cache` — change-detect cache (`$281A`) for the **altimeter terrain-height bar** (`$40E5` part 1) → `altimeter_terrain_cache`. `$2876` `gauge_281b_cache` — cache (`$281B`) for the **altimeter ship-height bar** (`$40E5` part 2, missile M3) → `altimeter_ship_cache`.
