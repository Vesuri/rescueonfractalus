...
# (digits no longer have a group of their own — their cells go through the dial registry.)
printf "\n=== cockpit scan per group (total %lu t/it, %lu scans) ===\n", \
  ($it ? g_fCockpit/$it : 0), g_fCockpitScans
# (no digits row: the digit cells are decoded through the dial registry now — the per-block digit
# registry served the per-rendered-frame $3FFA mirror, which is gone.)
...
printf "  skyline plot%6lu t (%4lu t/it)  <- g_fdSky: the edge plot itself\n", \
  g_fdSky, ($it ? g_fdSky/$it : 0)
printf "  p3clr+fillup%6lu t (%4lu t/it)\n", g_fdEdge,  ($it ? g_fdEdge/$it  : 0)
...
printf "  SUM of laps %6lu t (%4lu t/it)  vs g_fDirect %lu t/it (rest = entry/probe)\n", \
  (g_fdClear+g_fdSky+g_fdEdge+g_fdScan+g_fdFill+g_fdBand), \
  ($it ? (g_fdClear+g_fdSky+g_fdEdge+g_fdScan+g_fdFill+g_fdBand)/$it : 0), ($it ? g_fDirect/$it : 0)
