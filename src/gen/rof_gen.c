...
L_1935:;
    ...
    if (!cpu.C) goto L_1954;
    /* 1948 */
    bus_write(0xD006, 0xC8);
    /* 194d */
    bus_write(0xD007, 0x36);
L_1950:;
    /* 1950 */
    LDA(0x00);
    /* 1952 */
    terrain_scroll_reload = cpu.A;
L_1954:;
    /* 1954 */
    return;
}
...
void station_init(void) {
    ...
    /* 1995 */
    bus_write(0xD407, 0x30);
    /* 199a */
    bus_write(0xD208, 0x29);
    /* 199f */
    vbi_flags = 0x02;
    /* 19a3 */
    vbi_phase = 0x00;
...
L_19cd:;
    ...
    sync_flag = cpu.X;
    /* 19d5 */
    bus_write(0xD400, 0x3E);
    /* 19da */
    bus_write(0xD01D, 0x03);
    /* 19df */
    bus_write(0xD019, 0x34);
    /* 19e4 */
    bus_write(0xD012, 0x06);
    /* 19e9 */
    bus_write(0xD013, 0x0A);
    /* 19ec */
    LDA(0x7F);
    /* 19ee */
...
L_1c9a:;
    ...
    INY();
    /* 1c9d */
    bus_write(ZP_IND_Y(0x81), 0x90);
    /* 1c9f */
    INY();
    /* 1ca2 */
    bus_write(ZP_IND_Y(0x81), 0x2C);
    /* 1ca4 */
    goto L_1cd3;
L_1ca7:;
    /* 1ca7 */
...
void dli_handler_station(void) {
    ...
    station_sub_1E2A();
    /* 1e08 */
    INC_M(MEM_dl_bottom_index);
    /* 1e0a */
    DEC_M(MEM_grid_offset_b);
    /* 1e0c */
    if (!cpu.Z) goto L_1e1f;
    /* 1e0e */
    INC_M(MEM_grid_offset_a);
    /* 1e10 */
    LDA(0x0C);
    /* 1e12 */
    CMP(grid_offset_a);
...
L_1ea0:;
    /* 1ea0 */
    INC_M(MEM_sfx_reinit_gate);
    /* 1ea2 */
    if (!cpu.Z) goto L_1ea6;
    /* 1ea4 */
    INC_M(MEM_altitude_threshold);
L_1ea6:;
    /* 1ea6 */
    LDA(sfx_reinit_gate);
    /* 1ea8 */
...
void pmg_colors_station(void) {
    ...
    span_row_count = 0x07;
    /* 1f13 */
    INC_M(MEM_blit_row_counter);
    /* 1f15 */
    LDA(blit_row_counter);
    /* 1f17 */
    AND(0x07);
    /* 1f19 */
    TAY();
...
void loader_util__t6502(void) {
    ...
    frame_counter = 0x00;
    /* 3c06 */
    row_table_stride = 0xB5;
    /* 3c0a */
    player_speed = 0x32;
    /* 3c0e */
    row_table_base_lo = 0x47;
    /* 3c10 */
    LDA(0x00);
    /* 3c12 */
...
L_3c95:;
    ...
    /* 3c99 */
    INC_M(MEM_row_table_stride);
    /* 3c9b */
    if (!cpu.Z) goto L_3c9f;
    /* 3c9d */
    INC_M(MEM_player_speed);
L_3c9f:;
    /* 3c9f */
    DEC_M(MEM_row_table_base_lo);
    /* 3ca1 */
    LDA(0xFF);
    /* 3ca3 */
    CMP(row_table_base_lo);
    /* 3ca5 */
    if (!cpu.Z) goto L_3cae;
    /* 3ca7 */
    DEC_M(MEM_row_table_base_hi);
    /* 3ca9 */
    CMP(row_table_base_hi);
    /* 3cab */
    if (!cpu.Z) goto L_3cae;
    /* 3cad */
    return;
...
/* wait_frames @ $3CB1: PHA then falls into wait_timer_4c_frames spin (waits caller-set $4C frames), then PLA;RTS. Preserves A across a frame-wait */
/* faithful transliteration kept as the validation oracle; native wait_frames() lives in rof_native.c (see VALIDATE_FUNCS) */
void wait_frames__t6502(void) {
    /* 3cb1 */
    PHA();
    wait_timer_4c_frames(); return;
}

/* wait_frames_save_a @ $3CBE: PHA caller A, LDA #$3C, BNE(always) to wait_frames_set_count; delays 60 frames via frame_wait_count */
void wait_frames_save_a(void) {
    ...
    LDA(0x3C);
    /* 3cc1 */
    if (!cpu.Z) { wait_frames_set_count(); return; }
    wait_frames_1(); return;
}

/* wait_frames_1 @ $3CC3: PHA then LDA #$01 falling into wait_frames_set_count = wait 1 frame (accumulator preserved). NOT a colour clear (was misnamed clear_colors) */
void wait_frames_1(void) {
    /* 3cc3 */
    PHA();
    /* 3cc4 */
    LDA(0x01);
    wait_frames_set_count(); return;
}

/* wait_frames_2 @ $3CCA: PHA caller A, LDA #$02 then to wait_frames_set_count; delay 2 frames (RTCLOK_LOW vs frame_wait_count) */
...
/* wait_frames_5 @ $3CCF: PHA caller A, LDA #$05 then to wait_frames_set_count; delay 5 frames (RTCLOK_LOW vs frame_wait_count) */
...
/* wait_frames_10 @ $3CD4: PHA caller A, LDA #$0A then to wait_frames_set_count; delay 10 frames (RTCLOK_LOW vs frame_wait_count) */
...
/* wait_frames_20 @ $3CD9: PHA caller A, LDA #$14 then to wait_frames_set_count; delay 20 frames; falls through to game_entry */
...
L_3fc1:;
    /* 3fc1 */
    mem[MEM_audc_shadow_0+cpu.Y] = cpu.A;
    /* 3fc4 */
...
void draw_ah_ground_fill_p2__t6502(void) {
    ...
    LDY(ring_cur_4);
    /* 40b6 */
    CPX(canopy_pillar_x_cache);
    /* 40b9 */
    if (!cpu.Z) goto L_40c0;
    /* 40bb */
    CPY(canopy_pillar_y_cache);
    /* 40be */
    if (cpu.Z) goto L_40e4;
L_40c0:;
    /* 40c0 */
    canopy_pillar_x_cache = cpu.X;
    /* 40c3 */
    LDA(mem[(0x455B)+cpu.X]);
    /* 40c6 */
    TAX();
    /* 40c7 */
    canopy_pillar_y_cache = cpu.Y;
    /* 40cc */
    mem[(0x0E87)+cpu.Y] = 0x00;
    /* 40d1 */
    dl_y3 = 0x15;
L_40d3:;
    /* 40d3 */
...
void draw_altimeter_bars__t6502(void) {
    ...
    /* 40e8 */
    CPY(altimeter_terrain_cache);
    /* 40eb */
    if (cpu.Z) goto L_4105;
    /* 40ed */
    altimeter_terrain_cache = cpu.Y;
    /* 40f0 */
    LDA(0x00);
L_40f2:;
    /* 40f2 */
...
L_4105:;
    /* 4105 */
    LDY(viewport_bottom_row);
    /* 4108 */
    CPY(altimeter_ship_cache);
    /* 410b */
    if (cpu.Z) goto L_4130;
    /* 410d */
    altimeter_ship_cache = cpu.Y;
    /* 4110 */
...
/* vobj_step_down @ $41E8: Set $004C=1; advance $062F by +4, wrap at $DC bottom ($004C=0,$0D98=$F0,pos=$DC); calls wait_frames then erase row */
void vobj_step_down(void) {
L_41e8:;
    /* 41e8 */
    LDA(0x01);
    /* 41ea */
    frame_wait_count = cpu.A;
    /* 41ec */
    CLC();
    /* 41ed */
    LDA(0x04);
...
/* lock_on_indicator_dispatch @ $4225: X=$0043; if nonzero jump lock_on_indicator_return else lock_on_indicator_tick (state-driven object update branch) */
/* faithful transliteration kept as the validation oracle; native lock_on_indicator_dispatch() lives in rof_native.c (see VALIDATE_FUNCS) */
void lock_on_indicator_dispatch__t6502(void) {
    /* 4225 */
...
L_424d:;
    ...
    lock_on_indicator_fill_cells(); return;
}
...
L_42ef:;
    /* 42ef */
    player3_shape_mask = cpu.X;
    /* 42f2 */
    bar_col_threshold = cpu.A;
    /* 42f4 */
    LDX(player3_prev_rows);
    /* 42f7 */
...
L_43be:;
    /* 43be */
    CMP(bar_col_threshold);
    /* 43c0 */
    if (cpu.C) goto L_43c4;
    /* 43c2 */
    LDA(bar_col_threshold);
L_43c4:;
    /* 43c4 */
...
L_460a:;
    ...
    mem[0x3388] = cpu.Y;
    /* 4615 */
    special_state_color = cpu.Y;
    /* 4618 */
    INY();
    /* 4619 */
    cockpit_shields_cell = cpu.Y;
    /* 461c */
    INY();
    /* 461d */
    cockpit_shields_cell_hi = cpu.Y;
    /* 4620 */
    LDY(0x1E);
    /* 4622 */
    mem[0x33DF] = cpu.Y;
...
void event_sequence_dispatcher__t6502(void) {
    ...
    LDA(0x00);
    /* 4646 */
    LDY(event_pending_flag);
    /* 4649 */
    event_pending_flag = cpu.A;
    /* 464c */
    span_pixel_count = cpu.A;
    /* 464e */
...
L_4652:;
    /* 4652 */
    LDA(mem[MEM_attract_palette_src+cpu.Y]);
    /* 4655 */
    mem[MEM_display_param_0+cpu.Y] = cpu.A;
    /* 4658 */
    DEY();
    /* 4659 */
...
L_46b3:;
    ...
    copy_display_params_to_buffer();
    /* 46d0 */
    INC_M(MEM_clear_colors_done_003E);
    /* 46d4 */
    colpf0_value = 0x3A;
    /* 46d6 */
    LDA(0x2F);
...
void reset_pilot_state_if_no_2830__t6502(void) {
    ...
    LDA(0x00);
    /* 4961 */
    CMP(landing_target_queued_2830);
    /* 4964 */
    if (!cpu.Z) { clear_pilot_rescue_state(); return; }
    /* 4966 */
    landing_seq_flag = cpu.A;
...
L_4973:;
    /* 4973 */
    LDA(mem[MEM_display_param_0+cpu.Y]);
    /* 4976 */
    mem[MEM_attract_palette_src+cpu.Y] = cpu.A;
    /* 4979 */
    DEY();
    /* 497a */
...
void render_bcd_counter__t6502(void) {
    /* 49a2 */
    digit_dst_ptr_lo = 0xC5;
    /* 49a6 */
    digit_dst_ptr_hi = 0x32;
    /* 49a8 */
    LDA(score_display);
    /* 49ab */
    render_bcd_top_byte();
...
void obj_table_scan_replace__t6502(void) {
    /* 4e1c */
    obj_scan_fill_val = cpu.A;
    /* 4e1f */
    obj_scan_pass_count = cpu.Y;
L_4e22:;
    /* 4e22 */
    LDX(bus_read(0xD20A));
    /* 4e25 */
    obj_scan_start_idx = cpu.X;
L_4e28:;
    /* 4e28 */
    LDA(mem[MEM_terrain_height_map+cpu.X]);
    /* 4e2b */
    CMP(0x30);
    /* 4e2d */
...
L_4e37:;
    /* 4e37 */
    CMP(mem[MEM_obj_col_index+cpu.Y]);
    /* 4e3a */
    if (cpu.Z) goto L_4e48;
    /* 4e3c */
    DEY();
...
L_4e48:;
    ...
    TAX();
    /* 4e4d */
    CPX(obj_scan_start_idx);
    /* 4e50 */
    if (!cpu.Z) goto L_4e28;
L_4e52:;
    /* 4e52 */
    DEC_M(MEM_obj_scan_pass_count);
    /* 4e55 */
...
L_4e66:;
    /* 4e66 */
    LDA(mem[MEM_obj_col_index+cpu.X]);
    /* 4e69 */
    CMP(obj_active_idx);
    /* 4e6c */
    if (cpu.Z) goto L_4e7a;
...
void reset_flags_ff__t6502(void) {
    ...
    LDA(0xFF);
    /* 4e9a */
    p3_object_mode = cpu.A;
    /* 4e9c */
    object_index_signed = cpu.A;
    /* 4e9e */
    player3_dither_flag = cpu.A;
    /* 4ea1 */
    return;
}
...
L_4fb7:;
    ...
    /* 4fc0 */
    terrain_pen1_fade = cpu.A;
    /* 4fc2 */
    anim_counter_2 = cpu.A;
    /* 4fc4 */
    DEC_M(MEM_intro_phase_counter);
    /* 4fc7 */
...
void vbi_handler_flight__t6502(void) {
    ...
    LDA(event_active_flag);
    /* 503f */
    ORA(clear_colors_done_003E);
    /* 5041 */
    if (cpu.Z) goto L_505e;
    /* 5043 */
    INC_M(MEM_RTCLOK);
    /* 5045 */
    INC_M(MEM_event_pending_flag);
    /* 5048 */
...
L_50a3:;
    /* 50a3 */
    DEC_M(MEM_lock_on_indicator_tick_parity);
    /* 50a6 */
...
L_5178:;
    ...
    mem[0x0039] = cpu.X;
    /* 5181 */
    target_latched_idx = cpu.X;
    /* 5184 */
    INX();
    /* 5185 */
    mem[0x286B] = cpu.X;
    /* 5188 */
    object_anim_frame = cpu.X;
    /* 518c */
    vobj_row_count = 0x7C;
    /* 5190 */
    colpm2_shadow = 0x78;
...
L_51db:;
    ...
    /* 51ed */
    terrain_pen1_fade = mem[(0x084D)+cpu.Y];
    /* 51f2 */
    ...
    /* 51f9 */
    DEC_M(MEM_atmo_fade_countdown);
L_51fc:;
    /* 51fc */
    if (!cpu.Z) goto L_520f;
    /* 51fe */
    INC_M(MEM_atmo_fade_phase);
    /* 5201 */
    LDA(atmo_fade_phase);
    /* 5204 */
    LSR_A();
...
L_520f:;
    ...
    /* 5223 */
    LDA(level_cleared_flag);
    /* 5226 */
    if (cpu.Z) goto L_5230;
    /* 5228 */
    DEC_M(MEM_level_cleared_flag);
    /* 522b */
    if (!cpu.Z) goto L_5230;
    /* 522d */
...
L_5286:;
    ...
    LDA(bus_read(0xD20A));
    /* 5289 */
    CMP(static_dither_threshold);
    /* 528c */
    if (!cpu.C) goto L_5298;
    /* 5290 */
    bus_write(0xD016, 0x20);
    /* 5293 */
    LDA(0xF0);
    /* 5295 */
    goto L_529d;
...
void vbi_handler_standby(void) {
    ...
    /* 5326 */
    bus_write(0xD007, 0x39);
    /* 532b */
    bus_write(0xD004, 0xC5);
    /* 532e */
    LDA(0xC3);
    /* 5330 */
    bus_write(0xD005, cpu.A);
    ...
    INC_M(MEM_RTCLOK_LOW);
    /* 5335 */
    INC_M(MEM_attract_timer_sub);
    /* 5338 */
    if (!cpu.Z) goto L_533c;
    /* 533a */
    INC_M(MEM_attract_timer);
L_533c:;
    /* 533c */
    ...
    /* 5342 */
    LSR_M(MEM_lock_on_indicator_tick_parity);
    /* 5345 */
    if (cpu.C) { vbi_deferred_dispatch(); return; }
    /* 5347 */
    lock_on_indicator_tick();
    /* 534a */
    INC_M(MEM_lock_on_indicator_tick_parity);
...
L_543b:;
    ...
    /* 5441 */
    mem[MEM_sfx_env_prio_delta+cpu.Y] = cpu.A;
    /* 5444 */
    mem[MEM_sfx_env_prio_target+cpu.Y] = cpu.A;
    /* 5447 */
    mem[MEM_sfx_env_prio_step+cpu.Y] = cpu.A;
    /* 544a */
    mem[MEM_sfx_env_prio_phase+cpu.Y] = cpu.A;
    /* 544d */
    mem[MEM_sfx_env_freq_delta+cpu.Y] = cpu.A;
    /* 5450 */
    mem[MEM_sfx_env_freq_target+cpu.Y] = cpu.A;
    /* 5453 */
    mem[MEM_sfx_env_freq_step+cpu.Y] = cpu.A;
    /* 5456 */
    mem[MEM_sfx_env_freq_phase+cpu.Y] = cpu.A;
    /* 5459 */
...
L_5497:;
    ...
    /* 549c */
    LDA(mem[MEM_sfx_env_freq_step+cpu.Y]);
    /* 549f */
    if (cpu.Z) goto L_54d5;
    /* 54a1 */
    CLC();
    /* 54a2 */
    ADC(mem[MEM_sfx_env_freq_phase+cpu.Y]);
    /* 54a5 */
...
L_54b2:;
    ...
    CLC();
    /* 54bc */
    LDA(mem[MEM_sfx_env_freq_val+cpu.Y]);
    /* 54bf */
    ADC(mem[MEM_sfx_env_freq_delta+cpu.Y]);
    /* 54c2 */
    mem[MEM_sfx_env_freq_val+cpu.Y] = cpu.A;
    /* 54c5 */
    CMP(mem[MEM_sfx_env_freq_target+cpu.Y]);
    /* 54c8 */
...
L_54d5:;
    /* 54d5 */
    LDA(mem[MEM_sfx_env_prio_step+cpu.Y]);
    /* 54d8 */
    if (cpu.Z) goto L_5510;
    /* 54da */
    CLC();
    /* 54db */
    ADC(mem[MEM_sfx_env_prio_phase+cpu.Y]);
    /* 54de */
...
L_54eb:;
    ...
    CLC();
    /* 54f5 */
    LDA(mem[MEM_sfx_env_prio_val+cpu.Y]);
    /* 54f8 */
    ADC(mem[MEM_sfx_env_prio_delta+cpu.Y]);
    /* 54fb */
    AND(0x0F);
    /* 54fd */
    mem[MEM_sfx_env_prio_val+cpu.Y] = cpu.A;
    /* 5500 */
    CMP(mem[MEM_sfx_env_prio_target+cpu.Y]);
    /* 5503 */
...
L_5510:;
    ...
    /* 5515 */
    LDX(mem[MEM_sfx_env_event_id+cpu.Y]);
    /* 5518 */
    ring_push_marked();
L_551b:;
    /* 551b */
...
L_552f:;
    ...
    /* 5535 */
    LDA(mem[MEM_event_ring_0719+cpu.Y]);
    /* 5538 */
...
L_558d:;
    ...
    SBC(mem[(0x55DC)+cpu.Y]);
    /* 55a4 */
    sfx_pitch_0639 = cpu.A;
    /* 55a7 */
    bus_write(0xD204, cpu.A);
    /* 55aa */
    LDY(sfx_state_0637);
    /* 55ad */
    DEC_M(MEM_sfx_counter_063a);
    /* 55b0 */
...
L_55c2:;
    ...
    /* 55c9 */
    INC_M(MEM_sfx_state_0634);
L_55cc:;
    /* 55cc */
    INC_M(MEM_sfx_counter_063a);
L_55cf:;
    /* 55cf */
    sfx_state_0637 = cpu.Y;
    /* 55d2 */
    TYA();
L_55d3:;
    /* 55d3 */
...
void sfx_reorder_voice_slot__t6502(void) {
    ...
    AND(0x0F);
    /* 5621 */
    CMP(sfx_top_prio_val);
    /* 5624 */
    if (!cpu.C) goto L_5664;
    /* 5626 */
    if (!cpu.Z) goto L_562d;
    /* 5628 */
    CPY(sfx_top_voice_idx);
    /* 562b */
    if (!cpu.C) goto L_5664;
L_562d:;
    /* 562d */
    ...
    LDA(0x00);
    /* 5638 */
    mem[MEM_sfx_voice_reg_idx+cpu.X] = cpu.A;
    /* 563b */
    sfx_voice_write_freq_ctrl();
    /* 563e */
    goto L_5661;
...
void sfx_voice_write_freq_ctrl__t6502(void) {
    ...
    /* 567b */
    mem[(0xD1FE)+cpu.X] = mem[MEM_sfx_env_freq_val+cpu.Y];
    /* 567e */
    LDA(mem[MEM_sfx_env_prio_val+cpu.Y]);
    /* 5681 */
    AND(0x0F);
    /* 5683 */
    ORA(mem[MEM_sfx_voice_distortion+cpu.Y]);
    /* 5686 */
    mem[(0xD1FF)+cpu.X] = cpu.A;
L_5689:;
    /* 5689 */
...
L_5691:;
    ...
    /* 5697 */
    LDA(mem[MEM_sfx_env_prio_val+cpu.X]);
    /* 569a */
    AND(0x0F);
    /* 569c */
    CMP(sfx_scan_prio);
...
void sfx_event_load__t6502(void) {
    ...
    AND(0xF0);
    /* 582c */
    mem[MEM_sfx_voice_distortion+cpu.Y] = cpu.A;
    /* 582f */
    PLA();
    /* 5830 */
    AND(0x0F);
    ...
    mem[MEM_sfx_env_prio_val+cpu.Y] = cpu.A;
    /* 5838 */
    mem[MEM_sfx_env_freq_val+cpu.Y] = mem[(0x5716)+cpu.X];
    /* 583e */
    mem[MEM_sfx_env_prio_step+cpu.Y] = mem[(0x5737)+cpu.X];
    /* 5844 */
    mem[MEM_sfx_env_prio_delta+cpu.Y] = mem[(0x5758)+cpu.X];
    /* 584a */
    mem[MEM_sfx_env_prio_target+cpu.Y] = mem[(0x5779)+cpu.X];
    /* 5850 */
    mem[MEM_sfx_env_prio_phase+cpu.Y] = mem[(0x579A)+cpu.X];
    /* 5853 */
    ...
    /* 585e */
    mem[MEM_sfx_env_freq_delta+cpu.Y] = mem[(0x57DC)+cpu.X];
    /* 5864 */
    mem[MEM_sfx_env_freq_target+cpu.Y] = mem[(0x57E4)+cpu.X];
    /* 586a */
    mem[MEM_sfx_env_freq_phase+cpu.Y] = mem[(0x57EC)+cpu.X];
L_586d:;
    /* 586d */
...
L_58ad:;
    ...
    /* 58b0 */
    CMP(mem[MEM_high_score_b0+cpu.Y]);
    /* 58b3 */
...
L_58b9:;
    ...
    LDA(mem[(0x0600)+cpu.Y]);
    /* 58bc */
    mem[MEM_high_score_b0+cpu.Y] = cpu.A;
    /* 58bf */
    DEY();
    /* 58c0 */
...
L_58cb:;
    ...
    LDA(0xA8);
    /* 58cd */
    digit_dst_ptr_lo = cpu.A;
    /* 58d1 */
    digit_dst_ptr_hi = 0x36;
    /* 58d3 */
    LDA(ranking_level);
    /* 58d6 */
    bin_to_bcd();
...
L_58ff:;
    ...
    /* 5918 */
    digit_dst_ptr_lo = 0xF5;
    /* 591c */
    digit_dst_ptr_hi = 0x37;
    /* 591e */
    LDA(level_count_acc);
    /* 5921 */
...
L_5b81:;
    ...
    game_var_37F4 = cpu.A;
    /* 5b86 */
    name_entry_dirty = cpu.A;
    /* 5b89 */
    name_entry_cursor = cpu.A;
    /* 5b8c */
    wait_vcount_ge_7a();
    /* 5b91 */
    bus_write(0xD402, 0x2E);
    /* 5b94 */
    LDA(0x5E);
    /* 5b96 */
    bus_write(0xD403, cpu.A);
...
L_5ba5:;
    ...
    level_progress = cpu.A;
    /* 5bb2 */
    INC_M(MEM_name_entry_dirty);
L_5bb5:;
    /* 5bb5 */
    LDY(name_entry_cursor);
    /* 5bb8 */
    LDA(mem[(0x5CFA)+cpu.Y]);
    /* 5bbb */
    TAX();
    /* 5bbc */
    name_entry_cell_base = cpu.X;
    /* 5bbf */
    LDY(0x00);
L_5bc1:;
    /* 5bc1 */
    ...
    /* 5bcb */
    INC_M(MEM_name_entry_cursor);
    /* 5bce */
    LDA(name_entry_cursor);
    /* 5bd1 */
    CMP(0x08);
    /* 5bd3 */
    if (!cpu.C) goto L_5bb5;
    /* 5bd5 */
    LDA(name_entry_dirty);
    /* 5bd8 */
...
L_5be6:;
    ...
    music_init_state();
    /* 5bf2 */
    name_entry_shift_count = 0x06;
L_5bf5:;
    /* 5bf5 */
    LDY(name_entry_shift_count);
    /* 5bf8 */
    CPY(name_entry_cursor);
    /* 5bfb */
...
L_5c03:;
    ...
    /* 5c0d */
    DEC_M(MEM_name_entry_shift_count);
    /* 5c10 */
...
L_5c5f:;
    ...
    LDA(game_var_E4);
    /* 5c63 */
    altitude_threshold = cpu.Y;
    /* 5c65 */
    name_entry_cursor = cpu.Y;
    /* 5c68 */
    zp_flag_03 = cpu.Y;
    /* 5c6a */
    game_var_E4 = cpu.Y;
...
L_5cca:;
    ...
    /* 5cce */
    DEC_M(MEM_text_cell_col);
    /* 5cd1 */
    if (!cpu.N) goto L_5cd8;
    /* 5cd3 */
    INC_M(MEM_text_cell_col);
    /* 5cd6 */
...
void rle_unpack_to_07f9(void) {
    ...
    LDY(0x00);
    /* 5f02 */
    digit_dst_ptr_lo = cpu.Y;
L_5f04:;
    /* 5f04 */
    INC_M(MEM_digit_dst_ptr_lo);
L_5f06:;
    /* 5f06 */
...
L_5ffd:;
    ...
    /* 6000 */
    mem[MEM_scroll_accum_b0+cpu.Y] = cpu.A;
    /* 6003 */
    DEY();
    /* 6004 */
...
L_6065:;
    ...
    TXA();
    /* 6066 */
    mem[MEM_color_ring+cpu.Y] = cpu.A;
    /* 6069 */
    wait_frames();
    /* 606c */
    DEX();
...
L_641b:;
    ...
    wait_frames_10();
    /* 641e */
    INC_M(MEM_hud_field_678);
    /* 6421 */
    INC_M(MEM_sfx_env_freq_val);
    /* 6424 */
    LDA(hud_field_678);
    /* 6427 */
    CMP(0x03);
    ...
    /* 6433 */
    sfx_env_freq_target = 0xFF;
    /* 6438 */
    mem[0x06E6] = 0x01;
    /* 643b */
    mem[0x06E8] = 0x01;
    /* 643e */
    sfx_env_freq_phase = 0x01;
    /* 6441 */
    LDA(0x00);
    /* 6443 */
    mem[0x06F4] = cpu.A;
...
L_6478:; platform_tick_vbi(); platform_render_frame();
    ...
    LDY(0x0C);
    /* 648b */
    mem[MEM_sfx_env_prio_val+cpu.Y] = 0x0F;
    /* 648e */
    LDA(0xB4);
    /* 6490 */
    mem[MEM_sfx_env_freq_val+cpu.Y] = cpu.A;
...
L_64c4:;
    ...
    LDY(0x0B);
    /* 64de */
    mem[MEM_sfx_env_prio_val+cpu.Y] = 0x01;
    /* 64e1 */
    LDA(0x0E);
    /* 64e3 */
    mem[MEM_sfx_env_freq_val+cpu.Y] = cpu.A;
...
L_661a:;
    /* 661a */
    DEC_M(MEM_draw_iter_count);
    /* 661c */
    if (!cpu.N) goto L_6604;
L_661e:;
    /* 661e */
    DEC_M(MEM_draw_color_idx);
    draw_frame_guide_columns(); return;
}

/* draw_frame_guide_columns @ $6620: Sets row count $0092=0x55, $00A0=$009D+1; loops 85x: set_row_ptr_from_count then masked-plots cols $009C/$009D/$00A0 via plot_pixel_masked */
/* faithful transliteration kept as the validation oracle; native draw_frame_guide_columns() lives in rof_native.c (see VALIDATE_FUNCS) */
void draw_frame_guide_columns__t6502(void) {
    /* 6622 */
    draw_row = 0x55;
    /* 6624 */
    CLC();
...
void draw_symmetric_span_loop__t6502(void) {
    ...
    TAY();
    /* 6645 */
    ORA(mem[MEM_pixel_or_mask_tbl+cpu.Y]);
    /* 6648 */
    draw_pattern_byte = cpu.A;
L_664a:;
    /* 664a */
    ...
    fill_vertical_span();
    /* 6650 */
    DEC_M(MEM_draw_x_left);
    /* 6652 */
    INC_M(MEM_draw_x_right);
    /* 6654 */
    INC_M(MEM_draw_row_top);
    /* 6656 */
    DEC_M(MEM_draw_row_bottom);
    /* 6658 */
    DEC_M(MEM_span_row_count);
    /* 665a */
...
/* fill_vertical_span @ $669C: Iterates rows $009E..$009F: ptr $80/$81=tbl[$0084]; masked-plot col $009C via plot_pixel_masked and col $009D>>1 via plot_masked_pixel per row */
...
/* plot_pixel_masked @ $66D5: A=column: Y=A>>1 (byte idx), mask idx X from $0094(+8 if carry), tail-calls plot_masked_pixel to OR/AND pixel into row ptr $80 */
...
L_66dd:;
    /* 66dd */
    TAX();
    plot_masked_pixel(); return;
}

/* plot_masked_pixel @ $66DE: OR/AND a 2-bit pixel into screen byte via ($80) using mask tables $66E9(OR)/$66FB(AND) indexed by X */
/* faithful transliteration kept as the validation oracle; native plot_masked_pixel() lives in rof_native.c (see VALIDATE_FUNCS) */
void plot_masked_pixel__t6502(void) {
    /* 66de */
    LDA(bus_read(ZP_IND_Y(0x80)));
    /* 66e0 */
    ORA(mem[MEM_pixel_or_mask_tbl+cpu.X]);
    /* 66e3 */
    AND(mem[MEM_pixel_and_mask_tbl+cpu.X]);
    /* 66e6 */
    bus_write(ZP_IND_Y(0x80), cpu.A);
    /* 66e8 */
    return;
...
L_675a:;
    ...
    draw_x_left = cpu.A;
    /* 675e */
    digit_dst_ptr_lo = cpu.X;
    /* 6760 */
    LDY(0x05);
    /* 6762 */
    digit_dst_ptr_hi = cpu.Y;
L_6764:;
    /* 6764 */
    LDX(digit_dst_ptr_lo);
    /* 6766 */
...
void blit_numeric_readout__t6502(void) {
    ...
    draw_x_left = 0x1F;
    /* 67cf */
    LDA(sfx_rand_freq_hi1);
    /* 67d2 */
    glyph_ptr_from_index();
    /* 67d5 */
    LDA(sfx_rand_freq_lo1);
    /* 67d8 */
    glyph_ptr_shift3();
    /* 67db */
    LDA(sfx_rand_freq_hi2);
    /* 67de */
    glyph_ptr_from_index();
    /* 67e1 */
    LDA(sfx_rand_freq_lo2);
    /* 67e4 */
...
void emit_dl_coord_pairs__t6502(void) {
    ...
    PHA();
    /* 68d5 */
    LDX(row_table_base_hi);
    /* 68d7 */
    LDY(row_table_base_lo);
L_68d9:;
    /* 68d9 */
    LDA(mem[MEM_row_base_lo+cpu.X]);
    /* 68dc */
...
L_6964:;
    ...
    dl_bottom_index = cpu.Y;
    /* 696b */
    LDX(blit_row_counter);
    /* 696d */
    dl_lms_push_top();
    /* 6970 */
    blit_row_counter = cpu.X;
    /* 6972 */
    return;
}
...
void clear_slot_0c87_0d87(void) {
    /* 6a27 */
    DEC_M(MEM_terrain_scroll_reload);
    /* 6a29 */
    SEC();
    /* 6a2a */
    LDA(0x08);
    /* 6a2c */
    SBC(terrain_scroll_reload);
    /* 6a2e */
    TAY();
    /* 6a2f */
    LDA(0x00);
...
void step_accum_add_75__t6502(void) {
    ...
    add_multibyte_a1();
    /* 6a3d */
    scroll_accum_b3 = cpu.A;
    /* 6a3f */
    CMP(scroll_accum_prev);
    /* 6a41 */
...
L_6b0f:;
    ...
    /* 6b12 */
    mem[MEM_player_col_buf0+cpu.Y] = cpu.A;
    /* 6b18 */
    mem[MEM_player_col_buf1+cpu.Y] = mem[(0x0D33)+cpu.Y];
    /* 6b1e */
    mem[MEM_player_col_buf2+cpu.Y] = mem[(0x0E33)+cpu.Y];
    /* 6b21 */
    LDA(mem[(0x0F33)+cpu.Y]);
    /* 6b24 */
    mem[MEM_player_col_buf3+cpu.Y] = cpu.A;
    /* 6b27 */
    INY();
    /* 6b28 */
    CPY(0x59);
...
void gen_terrain_column__t6502(void) {
    ...
    random_terrain_height();
    /* 6b37 */
    mem[MEM_player_col_buf1+cpu.Y] = cpu.A;
    /* 6b3a */
    random_terrain_height();
    /* 6b3d */
    mem[MEM_player_col_buf2+cpu.Y] = cpu.A;
    /* 6b40 */
    random_terrain_height();
    /* 6b43 */
    mem[MEM_player_col_buf3+cpu.Y] = cpu.A;
...
L_6b75:;
    ...
    mem[(0x02C0)+cpu.X] = cpu.A;
    /* 6b78 */
    mem[(uint8_t)(MEM_scroll_accum_b0+cpu.X)] = cpu.A;
    /* 6b7a */
    DEX();
    /* 6b7b */
...
L_6b92:;
    ...
    ADC(0xE0);
    /* 6b98 */
    mem[MEM_obj_pos_table+cpu.Y] = cpu.A;
    /* 6b9b */
...
L_6bbe:;
    /* 6bbe */
    LDA(mem[MEM_obj_pos_table+cpu.X]);
    /* 6bc1 */
    draw_pattern_byte = cpu.A;
    /* 6bc3 */
    screen_ptr_hi = cpu.A;
...
L_6c2b:;
    ...
    if (cpu.N) goto L_6c4c;
    /* 6c3e */
    DEC_M(MEM_draw_row_ptr2_hi);
    /* 6c40 */
    if (!cpu.N) goto L_6c46;
    /* 6c44 */
    draw_row_ptr2_hi = 0x00;
L_6c46:;
    /* 6c46 */
    LDA(frame_counter);
    /* 6c48 */
...
void dli_launch_6d28(void) {
    ...
    bus_write(0xD40A, cpu.A);
    /* 6d2d */
    bus_write(0xD006, 0x3E);
    /* 6d32 */
    bus_write(0xD007, 0x3C);
    /* 6d37 */
    bus_write(0xD004, 0xC2);
    /* 6d3a */
    LDA(0xC0);
    /* 6d3c */
    bus_write(0xD005, cpu.A);
...
void dli_launch_grey_6d4f(void) {
    /* 6d51 */
    bus_write(0xD017, 0x06);
    /* 6d56 */
    bus_write(0xD016, 0x04);
    /* 6d5b */
...
L_6fc3:;
    ...
    ORA(0x80);
    /* 6fca */
    mem[MEM_terrain_height_map+cpu.X] = cpu.A;
    /* 6fcd */
    INX();
    /* 6fce */
    DEY();
...
void test_marked_neighbor__t6502(void) {
    ...
    TAY();
    /* 7057 */
    LDA(mem[MEM_terrain_height_map+cpu.Y]);
    /* 705a */
    if (!cpu.N) goto L_7066;
    /* 705c */
    TXA();
...
L_70ca:;
    ...
    AND(0x7F);
    /* 70e0 */
    mem[MEM_terrain_height_map+cpu.Y] = cpu.A;
L_70e3:;
    /* 70e3 */
    INY();
    /* 70e4 */
...
void reset_audctl_flags__t6502(void) {
    ...
    /* 70ef */
    sfx_note_timer = cpu.X;
    /* 70f2 */
    sfx_reinit_gate = cpu.X;
    /* 70f4 */
    DEX();
    /* 70f5 */
    sfx_seq_ptr = cpu.X;
...
L_723a:;
    ...
    /* 723d */
    mem[MEM_music_stream_ptr_lo+cpu.X] = cpu.A;
    /* 7240 */
    DEY();
    /* 7241 */
    DEX();
    ...
    INX();
    /* 7245 */
    music_note_timer = cpu.X;
    /* 7248 */
    bus_write(0xD208, cpu.X);
    /* 724b */
    INX();
    /* 724c */
    music_tempo_timer = cpu.X;
    /* 724f */
    mem[0x0655] = cpu.X;
    /* 7252 */
    return;
...
L_72b2:;
    ...
    ADC(mem[0x0099]);
    /* 72bf */
    music_stream_ptr_lo = cpu.A;
    /* 72c2 */
    if (!cpu.C) goto L_72c7;
    /* 72c4 */
    INC_M(MEM_music_stream_ptr_hi);
L_72c7:;
    /* 72c7 */
    LDA(bus_read(ZP_IND_Y(0x99)));
    /* 72c9 */
...
L_753f:;
    /* 753f */
    mem[MEM_terrain_height_max+cpu.Y] = cpu.A;
    /* 7542 */
    DEY();
    /* 7543 */
    if (!cpu.Z) goto L_753f;
    /* 7547 */
    row_table_stride = 0x70;
    /* 754b */
    player_speed = 0x10;
    /* 754f */
    row_table_base_lo = 0x9F;
    /* 7551 */
    LDA(0x11);
    /* 7553 */
    row_table_base_hi = cpu.A;
...
L_772b:;
    ...
    /* 779c */
    digit_dst_ptr_lo = 0x75;
    /* 77a0 */
    digit_dst_ptr_hi = 0x31;
    /* 77a4 */
    dl_src_index = 0x00;
    /* 77a6 */
    LDA(0x2F);
...
void pilot_render__t6502(void) {
    ...
    mem[0x004D] = 0x80;
    /* 785a */
    landing_target_queued_2830 = 0x80;
    /* 785d */
    mem[0x007A] = 0x80;
    /* 785f */
    LDA(mem[0x0079]);
...
L_7879:;
    ...
    ASL_A();
    /* 7893 */
    anim_counter_007B = cpu.A;
    /* 7897 */
    clear_sweep_delay_007C = 0xA5;
    /* 789b */
    clear_color_count_007D = 0x0F;
    /* 789f */
    altimeter_color_base = 0x80;
    /* 78a2 */
    altimeter_alt_ref = 0x80;
    /* 78a5 */
    mem[0x0079] = 0x80;
    /* 78a9 */
...
L_78fd:;
    /* 78fd */
    LDY(landing_seq_flag);
    /* 78ff */
    LDA(0x00);
    /* 7901 */
    landing_target_queued_2830 = cpu.A;
    /* 7904 */
    landing_seq_flag = cpu.A;
    /* 7906 */
    game_state = cpu.A;
...
void animate_clear_colors_timed__t6502(void) {
    ...
    /* 7a1b */
    LDY(anim_counter_007B);
    /* 7a1d */
    if (!cpu.Z) goto L_7a30;
    /* 7a1f */
    DEC_M(MEM_anim_counter_007B);
    /* 7a21 */
    LDX(0x19);
    /* 7a23 */
    ring_push_marked();
    ...
    wait_frames_10();
    /* 7a29 */
    ASL_M(MEM_clear_color_count_007D);
    /* 7a2b */
    ring_push_marked();
    /* 7a2e */
...
L_7a73:;
    ...
    /* 7a77 */
    INC_M(MEM_clear_color_count_007D);
    /* 7a79 */
    INC_M(MEM_clear_color_count_007D);
    /* 7a7b */
    LDA(clear_sweep_delay_007C);
    /* 7a7d */
    SBC(0x06);
    /* 7a7f */
    clear_sweep_delay_007C = cpu.A;
L_7a81:;
    /* 7a81 */
    DEY();
    /* 7a82 */
...
L_7b06:;
    ...
    /* 7b0c */
    INC_M(MEM_bcd_osc_dir);
L_7b0f:;
    /* 7b0f */
    INC_M(MEM_osc_step_counter);
    /* 7b12 */
    LDA(fresh_start_flag);
    /* 7b15 */
...
void setup_level_clear_state(void) {
    /* 7bc8 */
    mission_event_flag = 0xFF;
    /* 7bca */
    LDA(level_or_state);
    /* 7bcc */
    if (cpu.Z) goto L_7bd2;
    /* 7bd0 */
    flight_mode_state = 0x02;
L_7bd2:;
    /* 7bd2 */
    LDA(0x52);
    /* 7bd4 */
...
void plot_clipped_pixel__t6502(void) {
    ...
    /* 7d3a */
    LDX(terrain_pt_coord_a);
    /* 7d3c */
    LDY(terrain_pt_coord_b);
    /* 7d3e */
    CPY(0x6C);
    /* 7d40 */
...
void alien_knock_setup_loop__t6502(void) {
    ...
    /* 7ecc */
    sfx_voice_slot_a = cpu.X;
    /* 7ed0 */
    sound_table_idx = 0x09;
    /* 7ed3 */
    INX();
    /* 7ed4 */
    sfx_voice_slot_c = cpu.X;
    /* 7ed9 */
    sfx_voice_slot_b = 0x0B;
    /* 7edb */
    LDX(0x00);
    /* 7edd */
    sfx_used_val_prev = cpu.X;
    /* 7ee0 */
    sfx_shape_idx = cpu.X;
    /* 7ee3 */
    INC_M(MEM_sfx_state_0634);
    /* 7ee6 */
    sfx_state_0637 = cpu.X;
    /* 7ee9 */
    sfx_counter_063a = cpu.X;
    /* 7eec */
    bus_write(0xD208, cpu.X);
    /* 7ef1 */
    sfx_pitch_0635 = 0x20;
...
L_7f39:;
    ...
    ADC(0x01);
    /* 7f3b */
    zoom_step_count = cpu.A;
    /* 7f3e */
    LDX(0x04);
    /* 7f40 */
    sfx_voice_slot_a = cpu.X;
    /* 7f44 */
    sound_table_idx = 0x0F;
    /* 7f47 */
    INX();
    /* 7f48 */
    sfx_voice_slot_c = cpu.X;
    /* 7f4d */
    sfx_voice_slot_b = 0x12;
L_7f4f:;
    /* 7f4f */
    LDA(0x04);
L_7f51:;
...
L_7fb0:;
    /* 7fb0 */
    sfx_voice_val_a = cpu.A;
    /* 7fb3 */
    INC_M(MEM_sfx_voice_slot_b);
    /* 7fb5 */
    LDX(sfx_voice_slot_b);
    /* 7fb7 */
    LDA(mem[(0x81E8)+cpu.X]);
    /* 7fba */
    if (!cpu.Z) goto L_7fdd;
    /* 7fbc */
    LDX(sfx_voice_slot_c);
    /* 7fbf */
...
L_7fdd:;
    /* 7fdd */
    sfx_voice_val_b = cpu.A;
    /* 7fe0 */
    INC_M(MEM_sfx_shape_idx);
    /* 7fe3 */
    LDX(sfx_shape_idx);
    /* 7fe6 */
...
L_7feb:;
    ...
    INX();
    /* 7ff2 */
    CPX(sfx_used_val_prev);
    /* 7ff5 */
    if (cpu.Z) goto L_7ffa;
    /* 7ff7 */
    CPX(sfx_used_val_prev2);
L_7ffa:;
    /* 7ffa */
    if (cpu.Z) goto L_7feb;
    /* 7fff */
    sfx_used_val_prev2 = sfx_used_val_prev;
    /* 8002 */
    sfx_used_val_prev = cpu.X;
    /* 8005 */
...
L_808d:;
    /* 808d */
    draw_row_counter = cpu.X;
    /* 8090 */
    alien_shape_blit();
    /* 8093 */
    LDX(draw_row_counter);
    /* 8096 */
    INX();
    /* 8097 */
    CPX(0x2C);
...
L_80dc:;
    /* 80dc */
    LDA(mem[MEM_sfx_toggle_8F+cpu.Y]);
    /* 80df */
    screen_ptr_hi = cpu.A;
    /* 80e1 */
    TAX();
...
L_8d20:;
    ...
    /* 8d24 */
    mem[MEM_player_col_buf2+cpu.X] = cpu.A;
    /* 8d27 */
    INX();
L_8d28:;
    /* 8d28 */
...
L_8f20:;
    ...
    LDA(pitch_pos_lo);
    /* 8f22 */
    pitch_shadow_lo = cpu.A;
    /* 8f26 */
    pitch_shadow_hi = pitch_pos_hi;
L_8f28:;
    /* 8f28 */
    goto L_8f49;
L_8f2b:;
...
L_8fee:;
    ...
    CLC();
    /* 8fef */
    ADC(throttle_accum_lo);
    /* 8ff1 */
    throttle_accum_lo = cpu.A;
    /* 8ff3 */
    if (!cpu.C) goto L_8ff7;
    /* 8ff5 */
    INC_M(MEM_throttle_accum_hi);
L_8ff7:;
    /* 8ff7 */
    LDA(ground_proximity_flag);
    /* 8ff9 */
...
L_9134:;
    ...
    /* 9144 */
    level_ready_flag = cpu.Y;
L_9147:;
    /* 9147 */
    goto L_914e;
L_914a:;
    /* 914a */
    terrain_depth_frac = cpu.Y;
    /* 914c */
    LDA(0x4F);
L_914e:;
    /* 914e */
    terrain_depth_step = cpu.A;
    /* 9150 */
    LDA(roll_pos_lo);
    /* 9152 */
    ROL_A();
    ...
    ADC(0x05);
    /* 9161 */
    canopy_pillar_y_left = cpu.A;
    /* 9164 */
    SEC();
    /* 9165 */
    LDA(0x3A);
    /* 9167 */
    LDY(heading_freeze_flag);
    /* 916a */
...
L_9174:;
    ...
    /* 91a3 */
    ground_proximity_flag = cpu.A;
    /* 91a5 */
    LDX(lockon_rand_countdown);
    /* 91a8 */
    if (cpu.Z) goto L_91dc;
    /* 91aa */
    DEC_M(MEM_lockon_rand_countdown);
    /* 91ad */
    CPX(bus_read(0xD20A));
    /* 91b0 */
...
L_9228:;
    ...
    ADC(obj_delta_x_lo);
    /* 922c */
    obj_delta_x_lo = cpu.A;
    /* 922f */
    TYA();
    /* 9230 */
    ADC(obj_delta_x_hi);
    /* 9232 */
    obj_delta_x_hi = cpu.A;
    /* 9234 */
    LDY(0x00);
    /* 9236 */
    LDA(ring_cur_2);
...
L_9245:;
    ...
    ADC(obj_delta_y_lo);
    /* 9249 */
    obj_delta_y_lo = cpu.A;
    /* 924c */
    TYA();
    /* 924d */
    ADC(obj_delta_y_hi);
    /* 924f */
    obj_delta_y_hi = cpu.A;
    /* 9251 */
    LDA(object_anim_frame);
    /* 9253 */
...
L_9289:;
    /* 9289 */
    LDY(object_index_signed);
    /* 928b */
    if (cpu.N) goto L_92c7;
    /* 928d */
    DEY();
    /* 928e */
    object_index_signed = cpu.Y;
    /* 9290 */
...
L_92d1:;
    ...
    ring_head = cpu.Y;
    /* 92d7 */
    ring_cur_0 = mem[MEM_ring_pitch_lo+cpu.Y];
    /* 92dc */
    mem[MEM_ring_pitch_lo+cpu.Y] = pitch_pos_lo;
    /* 92e2 */
    ring_cur_1 = mem[MEM_ring_pitch_hi+cpu.Y];
    /* 92e7 */
    mem[MEM_ring_pitch_hi+cpu.Y] = pitch_pos_hi;
    /* 92ed */
    ring_cur_2 = mem[MEM_ring_roll_vel+cpu.Y];
    /* 92f2 */
    mem[MEM_ring_roll_vel+cpu.Y] = roll_velocity;
    /* 92f8 */
    ring_cur_3 = mem[MEM_ring_pillar_l+cpu.Y];
    /* 92fe */
    mem[MEM_ring_pillar_l+cpu.Y] = canopy_pillar_y_left;
    /* 9304 */
    ring_cur_4 = mem[MEM_ring_pillar_r+cpu.Y];
    /* 9307 */
    LDA(canopy_pillar_y_right);
    /* 930a */
    mem[MEM_ring_pillar_r+cpu.Y] = cpu.A;
    /* 930d */
    return;
}
...
L_9494:;
    /* 9494 */
    pitch_shadow_hi = cpu.A;
    /* 9496 */
    pitch_shadow_lo = cpu.Y;
L_9498:;
    /* 9498 */
    LDA(terrain_clearance);
    /* 949a */
...
void trigger_object_explosion__t6502(void) {
    ...
    LDA(0x02);
    /* 96ed */
    lock_on_indicator_complete = cpu.A;
    /* 96f0 */
    LDX(0x0F);
    /* 96f2 */
    ring_push_marked(); return;
...
void compute_obj_rel_angle_scale__t6502(void) {
    ...
    world_dx_hi = cpu.X;
    /* 97b2 */
    LDA(mul_multiplicand);
    /* 97b4 */
    ASL_A();
    /* 97b5 */
    if (!cpu.C) goto L_97b9;
    /* 97b7 */
    DEC_M(MEM_world_dx_hi);
L_97b9:;
    /* 97b9 */
    ASL_A();
    /* 97ba */
...
L_9825:;
    /* 9825 */
    LSR_M(MEM_mul_multiplicand);
    /* 9827 */
    ADC(mul_multiplicand);
L_9829:;
    /* 9829 */
    ASL_M(MEM_roll_mag_scaled);
    /* 982c */
...
void update_terrain_scanline_proj__t6502(void) {
    /* 9836 */
    map_x_scratch_hi = world_x_hi;
    /* 9839 */
    LDA(world_x_lo);
    /* 983c */
    LSR_M(MEM_map_x_scratch_hi);
    /* 983f */
    ROR_A();
    /* 9840 */
    LSR_M(MEM_map_x_scratch_hi);
    ...
    ROR_A();
    /* 984c */
    map_x_scratch_lo = cpu.A;
    /* 984f */
    map_x_lo = cpu.A;
    /* 9852 */
    map_x_mirror_lo = cpu.A;
    /* 9855 */
    LDA(map_x_scratch_hi);
    /* 9858 */
    map_x_hi = cpu.A;
    /* 985b */
    map_x_mirror_hi = cpu.A;
    /* 9861 */
    map_z_scratch_hi = world_z_hi;
    /* 9864 */
    LDA(world_z_lo);
    /* 9867 */
    LSR_M(MEM_map_z_scratch_hi);
    /* 986a */
    ROR_A();
    /* 986b */
    LSR_M(MEM_map_z_scratch_hi);
    ...
    ROR_A();
    /* 9877 */
    map_z_scratch_lo = cpu.A;
    /* 987a */
    map_z_lo = cpu.A;
    /* 987d */
    map_z_mirror_lo = cpu.A;
    /* 9880 */
    LDA(map_z_scratch_hi);
    /* 9883 */
    map_z_hi = cpu.A;
    /* 9886 */
    map_z_mirror_hi = cpu.A;
    /* 9889 */
    LDA(terrain_depth_step);
    /* 988b */
    CMP(0x40);
...
L_9891:;
    /* 9891 */
    scaled_depth_hi = cpu.A;
    /* 9894 */
    LDA(terrain_depth_frac);
    /* 9896 */
    ASL_A();
    /* 9897 */
    ROL_M(MEM_scaled_depth_hi);
    /* 989a */
    ASL_A();
    /* 989b */
    ROL_M(MEM_scaled_depth_hi);
    /* 989e */
    scaled_depth_lo = cpu.A;
    /* 98a1 */
    sample_terrain_height_bilerp();
    /* 98a4 */
    LDA(terrain_height_sample);
    ...
    ADC(0x00);
    /* 98ac */
    terrain_height_q2 = cpu.A;
    /* 98af */
    SEC();
    /* 98b0 */
    LDA(0x37);
    /* 98b2 */
    SBC(terrain_height_q2);
    /* 98b5 */
...
L_98b9:;
    /* 98b9 */
    viewport_top_row = cpu.A;
    /* 98bc */
    SEC();
    /* 98bd */
    LDA(scaled_depth_hi);
    /* 98c0 */
    SBC(terrain_height_sample);
    /* 98c2 */
    if (!cpu.C) goto L_98d1;
    /* 98c4 */
    terrain_clearance = cpu.A;
    /* 98c6 */
    LDA(landing_inhibit_flag);
    /* 98c9 */
...
void update_terrain_horizon_lr__t6502(void) {
    /* 992d */
    LSR_M(MEM_horizon_phase_toggle);
    /* 9930 */
    if (cpu.C) goto L_9936;
    /* 9932 */
    INC_M(MEM_horizon_phase_toggle);
    /* 9935 */
    return;
L_9936:;
    /* 9936 */
...
L_9958:;
    ...
    sample_terrain_height_bilerp();
    /* 9995 */
    LSR_M(MEM_terrain_height_sample);
    /* 9997 */
    LDA(terrain_height_sample);
    /* 9999 */
    LSR_A();
...
L_99bb:;
    ...
    horizon_left = cpu.A;
    /* 99be */
    CMP(horizon_left_prev);
    /* 99c1 */
    if (cpu.Z) goto L_99cd;
    /* 99c3 */
    horizon_left_prev = cpu.A;
    /* 99c6 */
    LDX(0x00);
    /* 99c8 */
    LDY(0x00);
...
L_9a1d:;
    ...
    horizon_right = cpu.A;
    /* 9a20 */
    CMP(horizon_right_prev);
    /* 9a23 */
    if (cpu.Z) goto L_9a35;
    /* 9a25 */
    horizon_right_prev = cpu.A;
    /* 9a28 */
    LDX(0x04);
    /* 9a2a */
    LDY(0x10);
...
void sample_terrain_height_bilerp__t6502(void) {
    ...
    TAY();
    /* 9a4a */
    mem[0x27F0] = mem[MEM_terrain_height_map+cpu.Y];
    /* 9a4d */
    INY();
    /* 9a4e */
    TYA();
    ...
    TAY();
    /* 9a57 */
    mem[0x27F1] = mem[MEM_terrain_height_map+cpu.Y];
    /* 9a5a */
    CLC();
    /* 9a5b */
    LDA(terrain_lerp_index);
    /* 9a5d */
    ADC(0x10);
    /* 9a5f */
    terrain_lerp_index = cpu.A;
    ...
    TAY();
    /* 9a6c */
    mem[0x27F2] = mem[MEM_terrain_height_map+cpu.Y];
    /* 9a6f */
    INY();
    /* 9a70 */
    TYA();
    ...
    TAY();
    /* 9a79 */
    mem[0x27F3] = mem[MEM_terrain_height_map+cpu.Y];
    /* 9a7c */
    LDX(map_x_lo);
    /* 9a7f */
    mem[0x27FA] = cpu.X;
...
void terrain_frame_setup__t6502(void) {
    ...
    /* 9e64 */
    obj_num1_lo = obj_num1_in_lo;
    /* 9e6a */
    obj_num1_hi = obj_num1_in_hi;
    /* 9e70 */
    obj_divisor_lo = obj_divisor_in_lo;
    /* 9e73 */
    LDA(obj_divisor_in_hi);
    /* 9e76 */
    obj_divisor_hi = cpu.A;
    /* 9e79 */
    LDX(0x22);
    /* 9e7b */
    LDY(0xB6);
...
L_9eab:;
    ...
    CLC();
    /* 9eb9 */
    LDA(mem[MEM_obj_num1_in_lo+cpu.Y]);
    /* 9ebc */
    ADC(draw_iter_count);
    /* 9ebe */
    mem[MEM_obj_num1_lo+cpu.Y] = cpu.A;
    /* 9ec1 */
    LDA(mem[MEM_obj_num1_in_hi+cpu.Y]);
    /* 9ec4 */
    ADC(scroll_accum_b0);
    /* 9ec6 */
    mem[MEM_obj_num1_hi+cpu.Y] = cpu.A;
    /* 9ec9 */
    SEC();
    /* 9eca */
    LDA(mem[MEM_obj_divisor_in_lo+cpu.Y]);
    /* 9ecd */
    SBC(scroll_accum_b1);
    /* 9ecf */
    mem[MEM_obj_divisor_lo+cpu.Y] = cpu.A;
    /* 9ed2 */
    LDA(mem[MEM_obj_divisor_in_hi+cpu.Y]);
    /* 9ed5 */
    SBC(scroll_accum_b2);
    /* 9ed7 */
    mem[MEM_obj_divisor_hi+cpu.Y] = cpu.A;
    /* 9eda */
    goto L_9f61;
L_9edd:;
    /* 9edd */
...
L_9f61:;
    ...
    SEC();
    /* 9f67 */
    LDA(mem[MEM_obj_num1_lo+cpu.Y]);
    /* 9f6a */
    SBC(scroll_accum_b1);
    /* 9f6c */
    mem[MEM_obj_num1_lo+cpu.Y] = cpu.A;
    /* 9f6f */
    LDA(mem[MEM_obj_num1_hi+cpu.Y]);
    /* 9f72 */
    SBC(scroll_accum_b2);
    /* 9f74 */
    mem[MEM_obj_num1_hi+cpu.Y] = cpu.A;
    ...
    SEC();
    /* 9f78 */
    LDA(mem[MEM_obj_divisor_lo+cpu.Y]);
    /* 9f7b */
    SBC(draw_iter_count);
    /* 9f7d */
    mem[MEM_obj_divisor_lo+cpu.Y] = cpu.A;
    /* 9f80 */
    LDA(mem[MEM_obj_divisor_hi+cpu.Y]);
    /* 9f83 */
    SBC(scroll_accum_b0);
    /* 9f85 */
    mem[MEM_obj_divisor_hi+cpu.Y] = cpu.A;
...
L_9fb2:;
    ...
    LDA(mem[MEM_terrain_height_map+cpu.X]);
    /* 9fc2 */
    mem[MEM_obj_height_sample+cpu.Y] = cpu.A;
    /* 9fc5 */
    SBC(terrain_scroll_reload);
    /* 9fc7 */
...
L_9fe6:;
    /* 9fe6 */
    mem[MEM_obj_num2_hi+cpu.Y] = cpu.A;
    /* 9feb */
    mem[MEM_obj_num2_lo+cpu.Y] = mem[0x00B5];
    /* 9fee */
    TXA();
    /* 9fef */
    mem[MEM_obj_col_index+cpu.Y] = cpu.A;
    /* 9ff2 */
    LDA(mem[MEM_obj_divisor_hi+cpu.Y]);
    /* 9ff5 */
...
L_a000:;
    ...
    LDA(0x00);
    /* a008 */
    SBC(mem[MEM_obj_num1_lo+cpu.Y]);
    /* a00b */
    mem[0x00B5] = cpu.A;
    /* a00d */
    LDA(0x00);
    /* a00f */
    SBC(mem[MEM_obj_num1_hi+cpu.Y]);
    /* a012 */
    CMP(mem[MEM_obj_divisor_hi+cpu.Y]);
    /* a015 */
    ...
    LDA(mem[0x00B5]);
    /* a01b */
    CMP(mem[MEM_obj_divisor_lo+cpu.Y]);
    /* a01e */
    if (!cpu.C) goto L_a037;
L_a020:;
    /* a020 */
...
L_a03d:;
    /* a03d */
    mem[MEM_obj_proj_flags+cpu.Y] = cpu.A;
    /* a040 */
    INY();
    /* a041 */
    CPY(0x2D);
...
L_a04a:;
    ...
    /* a04d */
    collapse_cur_obj = cpu.X;
    /* a050 */
    INY();
    /* a051 */
    LDA(mem[MEM_obj_proj_flags+cpu.X]);
    /* a054 */
    ...
    mem[0x00B5] = cpu.Y;
    /* a063 */
    LDY(collapse_cur_obj);
    /* a068 */
    mem[MEM_obj_divisor_hi+cpu.X] = 0x00;
    /* a06d */
    mem[MEM_obj_divisor_lo+cpu.X] = 0x20;
    /* a070 */
    LDA(0x00);
    /* a072 */
    mem[MEM_obj_proj_flags+cpu.X] = cpu.A;
    /* a075 */
    LDY(mem[0x00B5]);
    /* a077 */
...
L_a079:;
    ...
    LDY(collapse_cur_obj);
    /* a089 */
    mem[MEM_obj_divisor_hi+cpu.Y] = 0x00;
    /* a08e */
    mem[MEM_obj_divisor_lo+cpu.Y] = 0x20;
    /* a091 */
    LDA(0x00);
    /* a093 */
    mem[MEM_obj_proj_flags+cpu.Y] = cpu.A;
...
void build_view_transform_matrix__t6502(void) {
    ...
    signed_mul_8x16();
    /* a0b2 */
    obj_num1_in_lo = mem[0x00A8];
    /* a0b7 */
    obj_num1_in_hi = mem[0x00A9];
    /* a0bc */
    ...
    SBC(mem[0x00A8]);
    /* a0cd */
    obj_num1_in_lo = cpu.A;
    /* a0d0 */
    LDA(obj_num1_in_hi);
    /* a0d3 */
    SBC(mem[0x00A9]);
    /* a0d5 */
    obj_num1_in_hi = cpu.A;
    /* a0da */
    mem[0x00AA] = scroll_accum_b1;
    /* a0de */
    mem[0x00AB] = scroll_accum_b2;
    ...
    signed_mul_8x16();
    /* a0e7 */
    obj_divisor_in_lo = mem[0x00A8];
    /* a0ec */
    obj_divisor_in_hi = mem[0x00A9];
    /* a0f1 */
    mem[0x00AA] = draw_iter_count;
    /* a0f5 */
    mem[0x00AB] = scroll_accum_b0;
    ...
    LDA(mem[0x00A8]);
    /* a0ff */
    ADC(obj_divisor_in_lo);
    /* a102 */
    obj_divisor_in_lo = cpu.A;
    /* a105 */
    LDA(mem[0x00A9]);
    /* a107 */
    ADC(obj_divisor_in_hi);
    /* a10a */
    obj_divisor_in_hi = cpu.A;
    /* a10d */
    SEC();
    /* a10e */
    LDA(0x00);
    /* a110 */
    SBC(obj_divisor_in_lo);
    /* a113 */
    obj_divisor_in_lo = cpu.A;
    /* a116 */
    LDA(0x00);
    /* a118 */
    SBC(obj_divisor_in_hi);
    /* a11b */
    obj_divisor_in_hi = cpu.A;
    /* a11e */
    return;
...
void project_terrain_points__t6502(void) {
    ...
    LDY(0x00);
    /* a12c */
    mem[0x00AE] = mem[MEM_obj_divisor_lo+cpu.X];
    /* a131 */
    mem[0x00AF] = mem[MEM_obj_divisor_hi+cpu.X];
    /* a133 */
    LDA(mem[MEM_obj_num1_hi+cpu.X]);
    /* a136 */
    if (cpu.N) goto L_a142;
    /* a138 */
    mem[0x00B1] = cpu.A;
    /* a13d */
    mem[0x00B0] = mem[MEM_obj_num1_lo+cpu.X];
    /* a13f */
    goto L_a151;
L_a142:;
    /* a142 */
    ...
    LDA(0x00);
    /* a145 */
    SBC(mem[MEM_obj_num1_lo+cpu.X]);
    /* a148 */
    mem[0x00B0] = cpu.A;
    /* a14a */
    LDA(0x00);
    /* a14c */
    SBC(mem[MEM_obj_num1_hi+cpu.X]);
    /* a14f */
    mem[0x00B1] = cpu.A;
L_a151:;
    /* a151 */
...
L_a15f:;
    ...
    /* a16f */
    mem[MEM_obj_screen_x_hi+cpu.X] = 0x40;
    /* a174 */
    mem[MEM_obj_screen_x_lo+cpu.X] = 0x00;
    /* a177 */
    goto L_a202;
L_a17a:;
    /* a17a */
    LDA(0xC0);
    /* a17c */
    mem[MEM_obj_screen_x_hi+cpu.X] = cpu.A;
    /* a181 */
    mem[MEM_obj_screen_x_lo+cpu.X] = 0x00;
    /* a184 */
    goto L_a202;
...
L_a193:;
    ...
    ADC(div_quotient);
    /* a1ba */
    mem[MEM_obj_screen_x_lo+cpu.X] = cpu.A;
    /* a1bd */
    LDA(mem[0x00B5]);
    /* a1bf */
    ADC(0x00);
...
L_a202:;
    ...
    mem[0x00AF] = mem[MEM_obj_divisor_hi+cpu.X];
    /* a20e */
    LDA(mem[MEM_obj_num2_hi+cpu.X]);
    /* a211 */
    if (cpu.N) goto L_a21d;
    /* a213 */
    mem[0x00B1] = cpu.A;
    /* a218 */
    mem[0x00B0] = mem[MEM_obj_num2_lo+cpu.X];
    /* a21a */
    goto L_a22c;
L_a21d:;
    /* a21d */
    ...
    LDA(0x00);
    /* a220 */
    SBC(mem[MEM_obj_num2_lo+cpu.X]);
    /* a223 */
    mem[0x00B0] = cpu.A;
    /* a225 */
    LDA(0x00);
    /* a227 */
    SBC(mem[MEM_obj_num2_hi+cpu.X]);
    /* a22a */
    mem[0x00B1] = cpu.A;
L_a22c:;
    /* a22c */
...
L_a23a:;
    ...
    /* a24a */
    mem[MEM_obj_screen_y_hi+cpu.X] = 0x40;
    /* a24f */
    mem[MEM_obj_screen_y_lo+cpu.X] = 0x00;
    /* a252 */
    goto L_a2dd;
L_a255:;
    /* a255 */
    LDA(0xC0);
    /* a257 */
    mem[MEM_obj_screen_y_hi+cpu.X] = cpu.A;
    /* a25c */
    mem[MEM_obj_screen_y_lo+cpu.X] = 0x00;
    /* a25f */
    goto L_a2dd;
...
L_a26e:;
    ...
    ADC(div_quotient);
    /* a295 */
    mem[MEM_obj_screen_y_lo+cpu.X] = cpu.A;
    /* a298 */
    LDA(mem[0x00B5]);
    /* a29a */
    ADC(0x00);
...
L_a2dd:;
    /* a2dd */
    LDA(mem[MEM_obj_screen_x_hi+cpu.X]);
    /* a2e0 */
    if (cpu.N) goto L_a310;
    /* a2e2 */
    if (!cpu.Z) goto L_a317;
    /* a2e4 */
    LDA(mem[MEM_obj_screen_x_lo+cpu.X]);
    /* a2e7 */
    LSR_A();
    /* a2e8 */
    LSR_A();
    ...
    TAY();
    /* a2eb */
    LDA(mem[MEM_band_scroll_offset0+cpu.Y]);
    /* a2ee */
...
L_a2f0:;
    ...
    CLC();
    /* a2f1 */
    ADC(mem[MEM_obj_screen_y_lo+cpu.X]);
    /* a2f4 */
    mem[MEM_obj_screen_y_lo+cpu.X] = cpu.A;
    /* a2f7 */
    LDA(mem[MEM_obj_screen_y_hi+cpu.X]);
    /* a2fa */
    ADC(0x00);
    /* a2fc */
    mem[MEM_obj_screen_y_hi+cpu.X] = cpu.A;
...
L_a3da:;
    ...
    mem[0x25B4] = cpu.A;
    /* a3e3 */
    mem[0x25D2] = mem[MEM_obj_screen_x_hi+cpu.X];
    /* a3e9 */
    mem[0x25F0] = mem[MEM_obj_screen_y_lo+cpu.X];
    /* a3ef */
    mem[0x24E2] = mem[MEM_obj_screen_y_hi+cpu.X];
    /* a3f5 */
    mem[0x23E2] = mem[MEM_obj_height_sample+cpu.X];
    /* a3f8 */
    LDX(collapse_cur_obj);
    /* a3fb */
    LDA(mem[MEM_obj_proj_flags+cpu.X]);
    /* a3fe */
    AND(0x10);
...
L_a408:;
    ...
    dl_ptr_hi = cpu.A;
    /* a410 */
    screen_ptr_lo = mem[MEM_obj_screen_x_hi+cpu.X];
    /* a415 */
    screen_ptr_hi = mem[MEM_obj_screen_y_lo+cpu.X];
    /* a41a */
    encounter_count = mem[MEM_obj_screen_y_hi+cpu.X];
    /* a41c */
    LDA(mem[MEM_obj_height_sample+cpu.X]);
    /* a41f */
    row_count = cpu.A;
    /* a421 */
    LDX(0x00);
...
L_a433:;
    ...
    LDA(mem[0x28E7]);
    /* a436 */
    altimeter_color_base = cpu.A;
    /* a43c */
    altimeter_alt_ref = mem[0x28E8];
    /* a43f */
    LDA(mem[0x0079]);
    /* a441 */
...
L_a4a1:;
    ...
    /* a4ad */
    mem[0x28FE] = map_x_scratch_hi;
    /* a4b3 */
    mem[0x28FF] = map_z_scratch_lo;
    /* a4b9 */
    mem[0x2900] = map_z_scratch_hi;
    /* a4bf */
    mem[0x2901] = scaled_depth_lo;
    /* a4c5 */
...
L_a4ee:;
    ...
    LDA(row_span_seed);
    /* a4f2 */
    SBC(mem[MEM_band_scroll_offset0+cpu.Y]);
    /* a4f5 */
    CMP(0x80);
    /* a4f7 */
    ROR_A();
...
L_a59b:;
    /* a59b */
    LDA(map_cell_hit_marker);
    /* a59e */
    if (cpu.Z) goto L_a5b5;
    /* a5a0 */
    LDX(0x00);
    /* a5a2 */
    map_cell_hit_marker = cpu.X;
    /* a5a5 */
    LDA(0xF9);
L_a5a7:;
    /* a5a7 */
...
L_a5b5:;
    ...
    LDA(0x00);
    /* a5f5 */
    object_pos_x_hi = cpu.A;
    /* a5f7 */
    object_pos_y_hi = cpu.A;
    /* a5f9 */
    obj_delta_x_hi = cpu.A;
    /* a5fb */
    obj_delta_y_hi = cpu.A;
...
void terrain_plot_object__t6502(void) {
    /* a63b */
    LDY(mem[MEM_obj_col_index+cpu.X]);
    /* a63e */
    LDA(mem[(0x0A00)+cpu.Y]);
    /* a641 */
    if (cpu.Z) { terrain_plot_return(); return; }
    /* a643 */
    LDA(mem[MEM_obj_divisor_hi+cpu.X]);
    /* a646 */
    if (!cpu.Z) goto L_a64f;
    /* a648 */
    LDA(mem[MEM_obj_divisor_lo+cpu.X]);
    /* a64b */
    CMP(0x22);
    /* a64d */
...
L_a683:;
    ...
    /* a6ad */
    grid_slot_index = mem[MEM_obj_col_index+cpu.X];
    /* a6b2 */
    mem[0x0040] = 0x64;
    /* a6b4 */
    LDA(mem[MEM_obj_screen_x_hi+cpu.X]);
    ...
    if (!cpu.Z) goto L_a6c8;
    /* a6c1 */
    mem[0x003F] = mem[MEM_obj_screen_x_lo+cpu.X];
    /* a6c3 */
    LDA(mem[MEM_obj_screen_y_lo+cpu.X]);
    /* a6c6 */
    mem[0x0040] = cpu.A;
L_a6c8:;
    /* a6c8 */
...
void terrain_plot_object_a__t6502(void) {
    ...
    if (!cpu.Z) { terrain_obj_skip_return(); return; }
    /* a82e */
    plot_base_ptr_lo = 0xF9;
    /* a833 */
    plot_base_ptr_hi = 0xA6;
    /* a838 */
    plot_pixel_mask = 0xFF;
    /* a83d */
    plot_step_hi = mem[MEM_obj_divisor_hi+cpu.X];
    /* a842 */
    plot_step_lo = mem[MEM_obj_divisor_lo+cpu.X];
    /* a844 */
    LDA(mem[MEM_obj_screen_y_lo+cpu.X]);
    /* a847 */
    terrain_pt_coord_b = cpu.A;
    ...
    TAY();
    /* a84d */
    terrain_pt_coord_a = mem[MEM_obj_screen_x_lo+cpu.X];
    /* a84f */
    LDA(mem[MEM_obj_col_index+cpu.X]);
    /* a852 */
    TAX();
...
void terrain_plot_object_b__t6502(void) {
    ...
    if (!cpu.Z) { terrain_distance_clamp_return(); return; }
    /* a916 */
    plot_base_ptr_lo = 0xF1;
    /* a91b */
    plot_base_ptr_hi = 0xA7;
    /* a91e */
    LDA(mem[MEM_obj_height_sample+cpu.X]);
    /* a921 */
    AND(0x01);
    ...
    terrain_pt_coord_a = mem[MEM_obj_screen_x_lo+cpu.X];
    /* a931 */
    terrain_pt_coord_b = mem[MEM_obj_screen_y_lo+cpu.X];
    /* a936 */
    plot_step_hi = mem[MEM_obj_divisor_hi+cpu.X];
    /* a93b */
    plot_step_lo = mem[MEM_obj_divisor_lo+cpu.X];
...
L_a94a:;
    /* a94a */
    LDY(terrain_pt_coord_a);
    /* a94c */
    LDA(terrain_pt_coord_b);
    /* a94e */
    CMP(mem[MEM_terrain_height_max+cpu.Y]);
    /* a951 */
...
L_a965:;
    ...
    plot_pixel_mask = 0xFF;
    /* a97f */
    LDY(mem[MEM_obj_screen_y_lo+cpu.X]);
    /* a982 */
    LDA(mem[MEM_obj_screen_x_lo+cpu.X]);
    /* a985 */
    TAX();
...
void game_state_update__t6502(void) {
    /* a99c */
    DEC_M(MEM_lock_on_indicator_complete);
    /* a99f */
...
L_ab91:;
    /* ab91 */
    LSR_M(MEM_plot_step_hi);
    /* ab93 */
    ROR_M(MEM_plot_step_lo);
    /* ab95 */
    LSR_M(MEM_plot_step_hi);
    /* ab97 */
    ROR_M(MEM_plot_step_lo);
...
L_ac80:;
    /* ac80 */
    INC_M(MEM_target_hit_streak);
    /* ac83 */
    LDA(target_hit_streak);
    /* ac86 */
    CMP(0x02);
    /* ac88 */
...
L_acd0:;
    /* acd0 */
    ASL_M(MEM_dl_src_index);
    /* acd2 */
    ROL_A();
    /* acd3 */
    ROL_M(MEM_dl_src_index);
    /* acd5 */
    ROL_A();
    /* acd6 */
    terrain_scroll_reload = cpu.A;
...
L_ad34:;
    ...
    CLC();
    /* ad37 */
    ADC(scroll_accum_prev);
    /* ad39 */
    mem[0x00B5] = cpu.A;
    /* ad3b */
    LDA(mem[MEM_row_span_seed+cpu.X]);
    /* ad3e */
    ADC(scroll_accum_b3);
    /* ad40 */
...
L_ad4c:;
    ...
    SBC(scroll_accum_b3);
    /* ad58 */
    mem[MEM_band_scroll_offset0+cpu.X] = cpu.A;
    /* ad5b */
    DEX();
    /* ad5c */
...
void terrain_column_rasterize__t6502(void) {
    ...
    /* b35f */
    mem[MEM_terrain_height_max+cpu.X] = 0xFF;
    /* b362 */
    LDA(0x97);
L_b364:;
    /* b364 */
...
L_b38c:;
    ...
    CLC();
    /* b38d */
    ADC(mem[MEM_blit_color_src+cpu.Y]);
    /* b390 */
    ROR_A();
    /* b391 */
    CMP(0x2C);
...
L_b3af:;
    ...
    /* b3b1 */
    LDA(mem[MEM_blit_color_src+cpu.Y]);
    /* b3b4 */
    SEC();
    /* b3b5 */
    SBC(dl_ptr_hi);
...
L_b3e9:;
    /* b3e9 */
    mem[MEM_span_row_count+cpu.Y] = cpu.A;
    /* b3ec */
    LDA(row_count);
    /* b3ee */
    SEC();
...
L_b405:;
    ...
    /* b407 */
    LDA(mem[MEM_span_row_count+cpu.Y]);
    /* b40a */
    SEC();
    /* b40b */
    SBC(dl_ptr_hi);
...
L_b446:;
    ...
    SEC();
    /* b44e */
    SBC(mem[MEM_blit_color_src+cpu.Y]);
    /* b451 */
    CMP(0xFE);
    /* b453 */
...
/* compute_indicator_pos @ $B70C: Gated by flight_mode_state($0072)!=2 & $0063>=0: builds gauge/indicator pos in $0035 from coords $0064/65,$0066/67 (via b76f) & $0063; jiffy-paced via $0014; tail-calls b756 */
...
void init_B800(void) {
    /* b802 */
    bus_write(0x02F4, 0x04);
    /* b807 */
    bus_write(0x0230, 0x32);
    /* b80c */
    bus_write(0x0231, 0xB8);
    /* b811 */
...
/* wait_timer_4c_frames @ $3CB2: Sets RTCLOK_LOW=0, spins until RTCLOK_LOW($14)==caller-set frame_wait_count($4C); PLA+RTS. (The fixed $3C=60 count is loaded only by the separate entry $3CBE, not here.) */
/* faithful transliteration kept as the validation oracle; native wait_timer_4c_frames() lives in rof_native.c (see VALIDATE_FUNCS) */
void wait_timer_4c_frames__t6502(void) {
    /* 3cb4 */
    RTCLOK_LOW = 0x00;
    /* 3cb6 */
    LDA(frame_wait_count);
...
/* wait_frames_set_count @ $3CC6: STA frame_wait_count($4C); BNE(always, count!=0) falls into wait_timer_4c_frames spin loop */
void wait_frames_set_count(void) {
    /* 3cc6 */
    frame_wait_count = cpu.A;
    /* 3cc8 */
    if (!cpu.Z) { wait_timer_4c_frames(); return; }
...
L_3d7a:;
    ...
    DEY();
    /* 3d7b */
    mem[MEM_static_dither_threshold+cpu.Y] = cpu.A;
    /* 3d7e */
...
L_3dd7:;
    ...
    DEY();
    /* 3dd8 */
    mem[MEM_shield_or_damage+cpu.Y] = cpu.A;
    /* 3ddb */
...
L_3e34:;
    ...
    DEY();
    /* 3e35 */
    mem[MEM_landing_target_queued_2830+cpu.Y] = cpu.A;
    /* 3e38 */
...
L_3f59:;
    ...
    LDY(0x80);
    /* 3f5e */
    altimeter_color_base = cpu.Y;
    /* 3f61 */
    altimeter_alt_ref = cpu.Y;
    /* 3f64 */
    LDA(terrain_depth_step);
    /* 3f66 */
    CMP(0x40);
...
L_4178:;
    ...
    ADC(vobj_speed_frac);
    /* 417b */
    ADC(vobj_speed_accum);
    /* 417e */
    vobj_speed_accum = cpu.A;
    /* 4181 */
...
L_418c:;
    ...
    setup_dial_bar_draw();
    /* 419f */
    dial_draw_index = 0x1E;
    /* 41a1 */
    goto L_41a7;
L_41a4:;
    /* 41a4 */
    INC_M(MEM_life_counter);
L_41a7:;
    /* 41a7 */
    LDA(vobj_path_flag);
    /* 41a9 */
...
L_41bb:;
    ...
    TAX();
    /* 41c4 */
    mem[MEM_pm_shape_strip+cpu.Y] = 0x00;
    /* 41c7 */
    CPX(0x06);
    /* 41c9 */
...
L_41d4:;
    /* 41d4 */
    LDA(mem[MEM_vobj_shape_table+cpu.X]);
    /* 41d7 */
    indicator_light_state = cpu.A;
L_41d9:;
    /* 41d9 */
...
L_421c:;
    /* 421c */
    LDA(mem[MEM_vobj_shape_table+cpu.Y]);
    /* 421f */
    indicator_light_state = cpu.A;
    /* 4221 */
    DEX();
...
/* lock_on_indicator_fill_cells @ $4258: Lights all six lock-on indicator glyphs at once (mem[$3492-$3497]=$A9); fill-sweep opening frame + game-start (re)init (was misnamed game_sub_4258) */
/* faithful transliteration kept as the validation oracle; native lock_on_indicator_fill_cells() lives in rof_native.c (see VALIDATE_FUNCS) */
void lock_on_indicator_fill_cells__t6502(void) {
    /* 4258 */
    LDA(0xA9);
    /* 425a */
    LDY(0x05);
...
void lock_on_indicator_step__t6502(void) {
    ...
    /* 4269 */
    LDY(lockon_step_reload);
    /* 426c */
    anim_step_timer = cpu.Y;
    /* 426e */
    CMP(0x07);
    /* 4270 */
    if (!cpu.Z) goto L_4280;
    /* 4272 */
    LDA(lock_on_indicator_active);
    /* 4274 */
...
void lock_on_indicator_phase_advance__t6502(void) {
    ...
    /* 4293 */
    INC_M(MEM_lock_on_indicator_phase);
    /* 4296 */
    AND(0x0F);
    /* 4298 */
    CMP(0x07);
    ...
    LDA(0x06);
    /* 429e */
    DEC_M(MEM_lock_on_indicator_state);
L_42a0:;
    /* 42a0 */
    DEC_M(MEM_lock_on_indicator_state);
    /* 42a2 */
...
L_4402:;
    ...
    LDA(0xB7);
    /* 4404 */
    object_col_flag = cpu.A;
L_4406:;
    /* 4406 */
    CPX(bar_col_threshold);
    /* 4408 */
...
L_47e2:;
    /* 47e2 */
    LDX(mem[MEM_cockpit_msg_index+cpu.Y]);
    /* 47e5 */
    dl_y1 = cpu.Y;
    /* 47e7 */
    LDY(0x00);
L_47e9:;
    ...
    /* 47ef */
    LDA(mem[MEM_cockpit_msg_strings+cpu.X]);
    /* 47f2 */
    goto L_47f8;
L_47f5:;
    /* 47f5 */
...
L_47f8:;
    ...
    AND(0x7F);
    /* 47fc */
    mem[MEM_cockpit_msg_buffer+cpu.Y] = cpu.A;
    /* 47ff */
...
L_4f7e:;
    ...
    LDA(0x00);
    /* 4f96 */
    display_param_5 = cpu.A;
    /* 4f98 */
    LDY(0x05);
    /* 4f9a */
    game_over_latch = cpu.Y;
    /* 4f9c */
    init_game_vars_attract_timer(); return;
}
...
L_5607:;
    /* 5607 */
    mem[MEM_event_ring_0719+cpu.X] = cpu.A;
    /* 560a */
    DEX();
    /* 560b */
...
L_5a21:;
    ...
    random_alpha_index();
    /* 5a2d */
    sfx_rand_freq_hi1 = cpu.A;
    /* 5a30 */
    random_alpha_index();
    /* 5a33 */
    sfx_rand_freq_hi2 = cpu.A;
    /* 5a36 */
    random_digit();
    /* 5a39 */
    sfx_rand_freq_lo1 = cpu.A;
    /* 5a3c */
    random_digit();
    /* 5a3f */
    sfx_rand_freq_lo2 = cpu.A;
L_5a42:;
    /* 5a42 */
    LDA(0x10);
    /* 5a44 */
