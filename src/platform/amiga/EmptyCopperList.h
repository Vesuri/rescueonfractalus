...
// regardless of the (irrelevant) bitplane content; the unblanked border shows color00 too.
// The one active bitplane re-reads a tiny throwaway line via bpl1mod = -linebytes.  Shown black
// while the boot/standby build is still in progress (before g_standbyRevealReady), and reused
// for the death-cinematic salmon-hold → black fade (setColor00 poked from mem[$00D4]).
// renderFrame switches to the real lists once ready.
class EmptyCopperList : public CopperList {
public:
    EmptyCopperList();
    ~EmptyCopperList();
    void buildLayout(const Sprite& nullSprite);   // one-time: 1 plane (both pens equal), null sprites
