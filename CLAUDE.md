...
| `docs/high-score-restore-plan.md` | The high-score table + initials entry as BUILT (2026-08-18): the block (`src/rof_hiscore.c` + `_factory.h`), the restored SIO calls at `$5D86`/`$5D9D`, `RoF.hi` persistence (read before the takeover, written after it — `resload_SaveFile` is NOT reachable from the game), the entry screen's DL `$5E2E` + its shared text decoder, the full keyboard map, and §4's open items |
