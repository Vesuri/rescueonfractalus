...
light-grey bezel pixels are the stencil that clips the instrument sprites. This preserves all five
...
| **Flight (7)** | Startup config selects either the original **160×47** terrain stored as 2×2 pixels or the enhanced **320×94** physical renderer. The enhanced path expands the faithful `$260E` samples by one midpoint-displacement level into a stored 320-sample skyline, while one-bit procedural dots derive decorrelated X/Y phases from a deterministic column mix. Authored objects and overlays retain their source-sized 2×2 footprint. Both paths keep live terrain beneath the windscreen band and share its plane3 frame/punched-bar composite. Corner triangles remain separate **PMG sprites**. See [[flight-scene]]. |
...
When the same startup setting selects the enhanced flight terrain, the planet uses a separate
320×94 packed 2-bit buffer. `advance_object_positions` still performs every faithful `$1000`
write, but also supplies its 22 original 8.8-distance samples to a native rasterizer. That
rasterizer interpolates the three existing `$FF/$AA/$55` vertical boundaries at one-pixel X/Y
cadence and converts only changed physical rows to the viewport planes. The Copper advances to
the next stored row on every scanline. Rows 86-93 remain the ordinary doubled `$1810` transition
band, decoded separately, so the corner-triangle and `$6D67` palette semantics do not change.
